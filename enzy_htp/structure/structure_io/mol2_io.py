...
class Mol2Parser(StructureParserInterface):
    ...
    def get_file_str(cls, stru: Structure) -> str:
        """convert a Structure() to .mol2 file content. Only 1 residue unit is allowed in the stru"""
        pass # TODO
    ...
    def save_ligand(cls, outfile:str, ligand:Ligand) -> str:
        ...
        for aidx,atom in enumerate(ligand.atoms):
            chrg = atom.charge
            if chrg is None:
                _LOGGER.warning(f"Atom '{atom.name}' in ligand '{ligand.name}{ligand.idx}' has no charge! Setting to 0.0")
                chrg = 0.0 
