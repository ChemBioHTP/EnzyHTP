...
def build_residues(df: pd.DataFrame, keep: str = "first") -> Dict[str, Union[Residue, Ligand, Solvent, MetalUnit]]:
    ...
    for res_key, atoms in mapper.items():
        result[res_key] = Residue(residue_key=res_key, atoms=sorted(atoms, key=lambda a: a.atom_number))
...
def check_valid_pdb(pdbname: str) -> None:
    ...
    for idx, ll in enumerate(fs.lines_from_file(pdbname)):
        if not ll.isascii():
            _LOGGER.error(f"The PDB '{pdbname}' contains non-ASCII text and is invalid in line {idx}: '{ll}'. Exiting...")
            exit(1)
