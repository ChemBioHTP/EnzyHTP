"""Here is everything job manager need to know about ACCRE_R9 (Advanced Computing Center
for Research and Education; Rocky 9 version as updated in 2025) of Vanderbilt
...
class AccreR9(ClusterInterface):
    """
    The ACCRE_R9 interface
    """
    #############################
...
export GAUSS_SCRDIR=$TMPDIR/$SLURM_JOB_ID""",
    ...
    MULTIWFN_ENV = {
        "CPU" : """module load gcc/12.3  OpenMPI/4.1.5
...
{MULTIWFN_ENV['CPU']}""",
    ...
    RES_KEYWORDS_MAP = { 
        "core_type" : None,
        "nodes":"nodes=",
        "node_cores" : {"cpu": "tasks-per-node=", "gpu": "gres=gpu:"}, # for gpu need to include the architecture. e.g.: nvidia_rtx_a6000:1
        "job_name" : "job-name=",
        "partition" : "partition=",
    ...
    def _parser_res_dict(cls, res_dict):
        ...
        for k in new_dict:
            if k == "mem=":
                ...
                num_gpus = re.match(r"(?:.*:)?([0-9]+)", str(res_dict["node_cores"])).group(1)
                total_mem = round_by(float(mem_per_core_n_gb) * float(num_gpus), 0.1) # round up
