...
def sample_geometries( pdb: str, point: int, engine:str) -> List[Structure]:
    ...
    """
    IMPLEMENTATION:Dict = {
        'Amber_MD': _sample_amber_md        
    }

    if engine in IMPLEMENTATION:
        return IMPLEMENTATION[engine](pdb, point)
    else:
        raise UnsupportedMethod(f"{engine} is not supported for sample_geometries(). Supported methods include: {', '.join(list(IMPLEMENTATION.keys()))}")
