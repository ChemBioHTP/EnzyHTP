...
class Traj_calc:
    ...
    def From_keyword_find(cls, path, keyword="", if_pdb=1):
        ...
        for p_path in p_path_list:
            ...
            if len(p_prmtop) == 0:
                print(
                    "WARNING: Do not find prmtop file while nc file exists. Ignoring:",
                    p_path,
                )
                continue
            if len(p_prmtop) > 1:
            ...
            if if_pdb:
                ...
                if not os.path.exists(p_pdb):
                    print(
                        "WARNING: Do not find pdb file while nc and prmtop file exists. Ignoring:",
                        p_path,
                    )
                    continue
            else:
    ...
    def update_Radii(self, igb=5):
        ...
        try:
            run(
                "parmed -p " + prmtop_path + " -i ./tmp/parmed.in",
                check=True,
                text=True,
                shell=True,
                capture_output=True,
    ...
    def make_dry_frags(self, frag_str, igb=5, if_sol=0):
        ...
        if frag1_str[0] not in ("c.", "chain") or frag2_str[0] not in ("c.",
                                                                       "chain"):
            raise TrajCalcERROR(
                "ERROR: current version of make_dry_frags parser only take chain indexes. e.g. c. A+B:c. C"
            )
        ...
        if len(stru1.chains) == 0:
            raise TrajCalcERROR("ERROR: PDB" + self.pdb +
                                " do not contain chain: " + repr(frag1_chains))
        ...
        if len(stru2.chains) == 0:
            raise TrajCalcERROR("ERROR: PDB " + self.pdb +
                                " do not contain chain: " + repr(frag2_chains))
    ...
    def _pdb2prmtop_mmpbsa(pdb_path, igb, out_path=None):
        ...
        try:
            run(
                "tleap -s -f ./tmp/leap_pdb2prmtop.in",
                check=True,
                text=True,
                shell=True,
                capture_output=True,
    ...
    def run_MMPBSA(self, in_file="", overwrite=0, out_path=""):
        ...
        cmd = (Config.get_PC_cmd() + " python2 " +
               Config.Amber.MMPBSA.get_MMPBSA_engine() + " -O -i " + in_file +
               " -o " + out_path + " -sp " + self.prmtop + " -cp " +
               self.dc_prmtop + " -rp " + self.dr_prmtop + " -lp " +
               self.dl_prmtop + " -y " + self.nc)
        if Config.debug >= 1:
            print("running:   ", cmd)
        try:
