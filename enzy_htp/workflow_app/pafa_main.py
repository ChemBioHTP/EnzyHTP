...
def run_shrapnel(
    ...
    Args:
        tasks: Dict
            ...
            "task" in this list:
                ...
                "mod_aa_mainchain" : {"RES" : ["C", "CA", "N"], ...}, # optional
                },
            In detail, each key of this dict means:
                ...
                mod_aa_mainchain: Dict
                    Optional mapper for manually assigning mainchain atoms of
                    modified residues. Use this when automatic mainchain
                    detection would require missing NCAA connectivity. Example:
                    {"THN": ["C", "CA", "N"]}.
    ...
    for task in tasks:
        ...
        mod_aa_mainchain_mapper: Dict = task.get("mod_aa_mainchain", {})
        ...
        for wt_pdb in wt_pdb_list:
            ...
            for mut in mutants:
                child_tasks.append({
                    ...
                    "mod_aa_mainchain" : mod_aa_mainchain_mapper,
...
def _child_main(
    ...
    for i, task in enumerate(tasks):
        ...
        if prepared_stru is None:
            ...
            if mod_aa_mainchain_mapper:
                prepared_stru.assign_mod_aa_mainchain(mod_aa_mainchain_mapper)
            result_dict[task_uid]["prepared_stru"] = prepared_stru
            save_obj(result_dict, result_path)
        elif mod_aa_mainchain_mapper:
            prepared_stru.assign_mod_aa_mainchain(mod_aa_mainchain_mapper)
        ...
        if mutant_stru is None:
            ...
            if mod_aa_mainchain_mapper:
                mutant_stru.assign_mod_aa_mainchain(mod_aa_mainchain_mapper)
...
def main():
    ...
    tasks = [
        {   
        "wt" : [
            "init_stru/pafa_wt_4mer.pdb", 
            ],
        "mutants" : "WT, D473V", # You can also pre-screen mutants and load them by: `load_obj("stable_mutants.pickle")`
        "constraints" : [
            ],
        "ncaa_chrgspin" : {"ZN" : (2,1), "THN" : (-1,1)}, # assign chrgspin for NCAA. Note that this is optional because EnzyHTP can automatically detect chrgspin for NCAA.
        "mod_aa_mainchain" : {"THN" : ["C", "CA", "N"]}, 
        # assign mainchain atoms for THN. Note that this is optional because EnzyHTP can automatically detect mainchain atoms for THN.
        #  But to do that one need to use init_connectivity first on the structure.
        },
        {   
        "wt" : [
            "init_stru/pafa_d473v_4mer.pdb",   
            ],
        "mutants" : "WT, V473D", # You can also pre-screen mutants and load them by: `load_obj("stable_mutants.pickle")`
        "constraints" : [
            ],
        "ncaa_chrgspin" : {"ZN" : (2,1), "THN" : (-1,1)}, # assign chrgspin for NCAA. Note that this is optional because EnzyHTP can automatically detect chrgspin for NCAA.
        "mod_aa_mainchain" : {"THN" : ["C", "CA", "N"]},
