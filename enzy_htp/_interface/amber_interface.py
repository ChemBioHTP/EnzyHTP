...
class AmberInterface(BaseInterface):
    ...
    * Users should use this class as the only way to interact with any functionality
    ...
    def run_prepgen(self, in_file: str, out_file: str, mc_file: str, residue_name: str) -> None:
        ...
        # Handle output file path correctly - it might be in a different directory
        out_abs_path = os.path.abspath(out_file)
        if os.path.dirname(out_abs_path) == base_dir:
            # Output is in same directory as input
            out_relative = os.path.basename(out_file)
        else:
            # Output is in different directory - use relative path from base_dir
            out_relative = os.path.relpath(out_abs_path, base_dir)
        ...
        try:
            ...
            if not os.path.exists(out_relative):
                raise RuntimeError(f"prepgen failed to create output file {out_relative}. Output: {output_text}")
            
            if os.path.getsize(out_relative) == 0:
                raise RuntimeError(f"prepgen created empty output file {out_relative}. Output: {output_text}")
            
            # Clean up intermediate files generated by prepgen
            # Similar to antechamber cleanup - prepgen may generate these files
            intermediate_files = [
                "ATOMTYPE.INF",
                "NEWPDB.PDB", 
                "PREP.INF"
            ]
            fs.clean_temp_file_n_dir(intermediate_files)
