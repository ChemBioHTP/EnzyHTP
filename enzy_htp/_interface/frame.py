...
class FrameAtom:
    def __init__(
        self, idx: str, aname: str, resnum: int, resname: str, point, connections: List
    ):
        self.idx = idx
        self.aname = aname
        self.resnum = resnum
...
class Frame:
    ...
    def apply_H_caps(self, cap_mapper: Dict[int, FrameAtom]):
        ...
        for parent_idx, child in cap_mapper.items():
            ...
            cap_atoms.append(
                FrameAtom(
                    child.idx, "H", child.resnum, "CAP", start + vec, [parent.idx]
                )
            )
...
def parse_raw_pdb(lines: List[str]) -> pd.DataFrame:
    ...
    for ll in lines:
        ...
        result["point"].append(
            np.array([float(ll[30:38]), float(ll[38:46]), float(ll[46:54])])
        )
