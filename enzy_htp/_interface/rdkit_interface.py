...
class RDKitInterface(BaseInterface):
    ...
    def _supported_ftype(self, molfile:str) -> None:
        ...
        if ext not in self.config_.SUPPORTED_FTYPES:
            _LOGGER.error(f"The supplied file {molfile} has an unsupported extention '{ext}'. RDKitInterface supports: {', '.join(self.config_.SUPPORTED_FTYPES)}. Exiting...")
            exit( 1 )
    ...
    def _load_molecule(self, molfile:str,
                    sanitize:bool=False,
                    removeHs:bool=False,
