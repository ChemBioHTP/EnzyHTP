    ...
    for raw in flags:
        if len(raw) < 4:
            core._LOGGER.warning(
                f"The supplied flag '{raw}' is too short to be valid. Must be at least 4 characters long. Skipping mutaflag..."
            )
            continue
        orig_res = raw[0]
        if orig_res not in chem.ONE_LETTER_AA_MAPPER and orig_res != "X":
            core._LOGGER.warning(
                f"The supplied original residue '{orig_res}' is invalid. Skipping mutaflag..."
            )
            continue
        raw: str = raw[1:]
        ...
        if not chain_index.upper() or not chain_index.isalpha():
            core._LOGGER.warning(
                f"The supplied chain index '{chain_index}' is NOT a capitlized single letter. Maybe mal-formed."
            )
        raw = raw[1:]
        ...
        if target_res not in chem.ONE_LETTER_AA_MAPPER and target_res != "X":
            core._LOGGER.warning(
                f"The supplied original residue '{target_res}' is invalid. Skipping mutaflag..."
            )
            continue
        ...
        try:
            ...
            core._LOGGER.error(
                f"Unable to convert '{residue_index}' to an int(). Skipping mutation..."
            )
            continue
        decoded.append(
            Mutation(
                orig_residue=orig_res,
...
def get_all_combinations(
    curr_state: List[Tuple[str, str, int]],
    ...
    for (chain, res_name, index) in curr_state:
        ...
        if len(res_name) != 1:
            core._LOGGER.warning(
                f"get_all_combinations() expects residue states to have one letter residue names. Continuing.."
            )
            continue
        ...
        for other in chem.one_letters_except(res_name):
            all_combos.append(
                Mutation(
                    orig_residue=res_name,
                    chain_index=chain,
                    residue_index=index,
                    target_residue=other,
                )
            )
