...
def setup_params():
	...
	parser.add_argument('--is_class',  action='store_true', help='Flag to indicate if file contains a class. False by default.')
	parser.add_argument('--description', required=True, type=str, help='description string for the module.')
	return parser.parse_args()

def get_dev_name() -> str:
	result = os.popen('git config user.name').read().splitlines()
	if not result:
		raise Exception("Developer's name is not set. Set with 'git config user.name \"first_name last_name\"")
	return result[0]

def get_dev_email():
	result = os.popen('git config user.email').read().splitlines()
	if not result:
		raise Exception("Developer's email is not set. Set with 'git config user.email \"<uname>@email.com\"")
	return result[0]
