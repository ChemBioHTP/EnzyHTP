...
class PDB():
    ...
    def get_distance(self, resi_id_1: int,resi_id_2: int, chain_id: str) -> float:
        pass
        self.get_stru()
    ...
    def random_mut_generator(self, resi_id: int, chain_id: str) -> list:
        """return a mutaflag list for random generation
        
        Reads input of desired residues id and residues chain id. The outputs
        generate the MutaFlag.

        Args:
            resi_id: the residues id from the pdb objects.
                The example input 12
            chain_id: the chain id from the pdb objects in the format of string.
                The example input 'A'

        Returns:
            The MutaFlag of list. The example output W12AY. 
        """
        MutaFlag = []
        resi_allowed_list = [] #first create the list for the resi changing candidate
        self.get_stru() #get structure
