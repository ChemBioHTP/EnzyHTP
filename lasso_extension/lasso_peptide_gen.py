...
def clean_PDB(outfile_name: str) -> None:
    ...
    with open(f"lasso_extension/output_structures/{outfile_name}.pdb", "r", encoding="utf-8") as pdb:
        lines = pdb.readlines()
    lines = lines[2:] # trim first two lines

    lines = list(filter(lambda line: not (line.startswith("CONECT") or line.startswith("MASTER")), lines))
