...
def correct_line(o_line : str, s_line : str):
    """Cleans a single output line based on the corresponding scaffold line
    
    Args:
        o_line: The output PDB line generated by OpenBabel
        s_line: The corresponding line from the scaffold
    """
    return 
...
def clean_PDB(outfile_name: str, scaffold_file : str, extender_file : str) -> None:
    ...
    for j in range(i, len(lines) - e_length-2):
        ...
        lines[j] = lines[j][:13] + atom_name + e_name.ljust(7) + str(res_num).ljust(6) + lines[j][-49:-14] + "\n"

    j += 1

    for k in range(j, len(lines) - 1):
        res_num = (k-i)//e_length+n_residues+1
        lines[k] = lines[k][:20] + str(res_num).rjust(6).ljust(10) + lines[k][-49:-14] + "\n"
