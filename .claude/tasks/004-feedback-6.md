...
1. `af2_predict` need to have the `non_armer_core_type` option working. It should only take effect when cluster_job_config == None and run() is used. Similarly, the command in `_build_..._command` methods should have reflect the core type difference.
