...
4. For these unit tests:
   ...
   ```
5. For `make_job()` in alphafold_interface:
   1) Reference `make_job` from `GaussianSinglePointEngine` for how `env_settings` and `sub_script_path` is handled. `env_settings` should be determined by the target cluster. Reference the script above and expand the AccreR9 class
   2) You have `sub_dir=str(out_dir)` but is every path in your cmd support it? If they are just absolute paths, just use the current dir for submission.
