...
def gather_output(num_group, dir_list = None):
    if not dir_list:
        dir_list = [f".//group_{j}" for i in substrate_list for j in range(num_group)]
    out_dir = "./"
    for wk_dir in dir_list:
        result_path = f"{wk_dir}/Mutation.dat"
        run_cmd(f"cat {result_path} >> {out_dir}total_result.dat")
...
def do_completed(job_dirs):
    ...
    for job_dir in job_dirs:
        ...
        script_path = f"{job_dir}/htp_md.py"
        with open(script_path) as f:
            mutation_list = re.search(r"for i in range\((\d+)\)", f.read()).group(1)
        total_num = eval(mutation_list)
        if os.path.exists(data_path):
            with open(data_path) as f:
                current_num = len(re.findall("TAG", f.read()))
            if current_num == total_num:
...
def generate_input_dirs_repeat_md(starting_pdb: str, num_repeat: int, num_group: int= 10):
    """generate working dirs for a list of mutations"""
    work_dir = f"./KE07/"
    mkdir(work_dir)

    num_rep_each_group: list = assign_task_num(num_repeat, num_group)
    for i, mut_rep in enumerate(num_rep_each_group):
        ...
        run(f'sed -i "s/XXX/{mut_rep}/" {script_path}', check=True, text=True, shell=True, capture_output=True)
...
def main():
    num_group = 10
    # == dir gen ==
    # generate_input_dirs_repeat_md("KE_07_S.pdb", 100, num_group)
    assign_partition_single_sub(range(5,10), "maxwell")
    launch_jobs_single_sub(range(0,10))
    ...
    # print(*do_completed([f"group_{i}" for i in range(10)]), sep="\n")
