...
def main():
    ...
    for i in range(25):
        of.write(
            ("Round: " + str(i) + line_feed).encode("utf-8")
        )  # Remove water and ion in original crystal file 											(self.path)
        # --- Preparation ---
        pdb_obj = PDB(
            "./FAcD-FA-ASP.pdb", wk_dir="./FAcD_test" + str(i)
        )  # Initiate with the file from protein data bank 											(self.path)
        pdb_obj.rm_wat()
        of.write(
            (
                "Preparation: " + str(datetime.datetime.now() - starttime) + line_feed
            ).encode("utf-8")
        )  # Remove water and ion in original crystal file 											(self.path)
        ...
        exit_code = pdb_obj.PDBMin(
            engine="Amber_pmemd_gpu"
        )  # Minimization 																				(self.path)
        if exit_code == 1:
            continue
        of.write(
            (
        ...
        pdb_obj.PDB2FF(
            local_lig=0, ifsavepdb=1
        )  # Generate parameter files *savepdb save the exact structure use in MD for future analysis 	(self.path)
        pdb_obj.PDBMD(
            tag=Muta_tag, engine="Amber_pmemd_gpu", equi_cpu=1
        )  # Run MD 																					(self.nc)
        of.write(
            ("MD: " + str(datetime.datetime.now() - starttime) + line_feed).encode(
                "utf-8"
            )
        )  # Remove water and ion in original crystal file 											(self.path)
        # sample
        pdb_obj.nc2mdcrd(
            point=100
        )  # Sample from trajactory 																	(self.mdcrd)
        ...
        pdb_obj.PDB2QMCluster(
            atom_mask, g_route=g_route, ifchk=1
        )  # Run QM cluster calculation																(self.qm_cluster_out, self.qm_cluster_chk)
        of.write(
            (
                "QM Cluster: " + str(datetime.datetime.now() - starttime) + line_feed
            ).encode("utf-8")
        )  # Remove water and ion in original crystal file 											(self.path)
        pdb_obj.get_fchk(
            keep_chk=0
        )  # Save fchk files for analysis																(self.qm_cluster_fchk)
        ...
        Es = pdb_obj.get_field_strength(
            E_atom_mask, a1=a1, a2=a2, bond_p1="center"
        )  # Run Field Strength analysis
        of.write(
            (
                "Get field strength: "
                + str(datetime.datetime.now() - starttime)
        ...
        Dipoles = PDB.get_bond_dipole(
            pdb_obj.qm_cluster_fchk, a1qm, a2qm
        )  # Run Bond Dipole Moment analysis
        of.write(
            (
                "Get bond dipole: "
                + str(datetime.datetime.now() - starttime)
        ...
        r2 = pdb_obj.stru.chains[ord(pdb_obj.MutaFlags[0][1]) - 65][
            int(pdb_obj.MutaFlags[0][2]) - 1
        ]
        ...
        write_data(
            pdb_obj.MutaFlags,
            {"E": Es, "Bond Dipole": Dipoles, "Distance": Dist},
            data_output_path,
        )  # Current data: Mutation - MD geometry - QM cluster wavefunction = Field strength at bond - Bond dipole moment
        of.write(
            (
                Muta_tag
                + " finish: "
