...
def test_mutated_name():
    ...
    actual = mut.engine.mutated_name(ONE_RES, "dne", [mut.Mutation(orig='P', target='G', chain_id='A', res_num=1)])
...
def test_mutated_name_can_deduce():
    ...
    ONE_RES:str=f"{DATA_DIR}/one_res.pdb"
    target = "dne/one_res_P1G.pdb"
    actual = mut.engine.mutated_name(ONE_RES, "dne", [mut.Mutation(orig='X', target='G', chain_id='A', res_num=1)])
    assert  actual == target
