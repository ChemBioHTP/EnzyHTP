...
def test_mutated_name():
    ...
    actual = mut.engine.mutated_name(
        ONE_RES, "dne",
        [mut.Mutation(orig='P', target='G', chain_id='A', res_num=1)])
...
def test_mutated_name_can_deduce():
    ...
    actual = mut.engine.mutated_name(
        ONE_RES, "dne",
        [mut.Mutation(orig='X', target='G', chain_id='A', res_num=1)])
    assert actual == target
...
def test_mutate_pdb_specified_mutation_tleap():
    ...
    mutated = mut.mutate_pdb(
        ONE_RES, 1,
        [mut.Mutation(orig='P', target='G', chain_id='A', res_num=1)], None,
        "tleap", None, 100)
    assert os.path.exists(target)
