...
def mutant_design_workflow(stru_parser: StructureParser):
    ...
    stru_prepare_problems: List[StruProblem] = eh.detect_stru_problems(
        raw_stru
    )  # register preparer with required preparation steps (like a builder pattern)
    preparer = eh.preparation.StructurePreparer(
        stru_prepare_problems
    )  # configure preparer to avoid massive function parameters ? should we use an object here?
    preparer.add_problem(
        MissingLigand('path-to-ligand',
                      information_of_docking,
                      solver=eh.external_interface.Rosetta())
    )  # the Problem classes have a common interface called solve that preparer do not need to know what kind of problem it is dealing. This design is because structural problem is easily expanding in type. In this design the problem carry the information of how to solve it in itself.
    preparer.delete_problem(
        'stoichiometry'
    )  # one can edit the method of dealing with different problems
    preparer.problems['protonation'].set_ph(
        7.0
    )  # currently preparer is nothing more than a special dictionary to contain problems

    prepared_stru = preparer.run(
        raw_stru
    )  # this structure should be have complete topology data and initial coordinate

    muta_flag = eh.mutation.assign_mutation(
        'pattern', prepared_stru
    )  # this muta_flag is coupled with the specific struture as the mutator input
    mutator = eh.mutation.LeapMutator(
    )  # in the future there will be more mutators using different methods to mutate
    mutator.set_mutation(muta_flag)

    mutant_stru = mutator.run(
        prepared_stru
    )  # its better to design unified interface for sciencific problem solver
    ...
    for stru in stru_ensemble:
        ele_stru_engine = eh.electronic.QMEngine(
            eh.external_interface.Gaussian())
        ele_stru_engine.software.scf_fail_solver.add_keyword(
            'scf=qc'
        )  # these inteface objects can be configured with their specific problems
...
def mutant_design_lambda_dynamic():
    ...
    alchemical_mutator = eh.mutation.alchemical.LambdaDynamicMutator(
    )  # alchemical mutator gives free energy differences of defined start/end state
    alchemical_mutator.set_mutation(
        muta_flag)  # alchemical mutator also have mutator interfaces
    end_wt_stru = eh.structure.operation.undock(prepared_stru)

    mutant_stru, ddG_mut_start_end = alchemical_mutator.run(
        prepared_stru, end_wt_stru)


def mutant_design_workflow_w_object(parser, preparer, muta_flags, mutator,
                                    sampler, energy_engine, ranker):
    '''
...
def mutant_design_workflow_wo_object():
    ...
    problems.append(
        'docking'
    )  # code for configuring each step is coupled with the workflow code
    eh.preparation.prepare_stru(
        raw_stru,
        problems,
        docking_ligand_path='path-to-ligand',
        protonation_ph=7.0
