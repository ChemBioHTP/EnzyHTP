...
def test_get_target_model():
    ...
    target_model_df, target_model_ter_df = sp._get_target_model(test_pdb.df,
                                                                model=0)
...
def test_resolve_missing_chain_id_simple():
    ...
    def get_chains(fname) -> Dict[str, Chain]:
        ...
        target_model_df, target_model_ter_df = sp._get_target_model(
            input_pdb.df, 0)
...
def test_get_legal_pdb_chain_ids():
    ALL_NAMES = list(string.ascii_uppercase) + list(
...
def test_resolve_alt_loc_first():
    ...
    answer_mdl_pdb.read_pdb(answer_mdl)
    answer_df = pd.concat(
        (answer_mdl_pdb.df['ATOM'], answer_mdl_pdb.df['HETATM']),
        ignore_index=True)
...
def test_build_residue():
    ...
    assert len(
        list(filter(lambda x: x.rtype is chem.ResidueType.LIGAND,
                    all_residue))) == 2
    sample = all_residue[0]
    # test if parent and children are will set
    assert filter(
        lambda x: (x.residue.idx, x.residue.name) == (sample.idx, sample.name),
        sample.atoms)
...
def test_categorize_residue_canonical():
    ...
    for res_key, atoms in atom_mapper.items():
        res_mapper[res_key[0]].append(
            Residue(int(res_key[1]), res_key[2], atoms))

    sp._categorize_pdb_residue(res_mapper)
    all_residue = list(itertools.chain.from_iterable(res_mapper.values()))
    assert len(
        list(
            filter(lambda x: x.rtype is not chem.ResidueType.CANONICAL,
                   all_residue))) == 0
...
def test_categorize_residue_metal():
    ...
    res_mapper[res_key[0]].append(
        Residue(int(res_key[1]), res_key[2], [zn_atom]))
...
def test_categorize_residue_noncanonical():
    ...
    assert len(
        list(
            filter(lambda x: x.rtype is chem.ResidueType.NONCANONICAL,
                   all_residue))) == 1
...
def test_get_structure_simple():
    ...
    chain_B_target = [
        'B.GLY.12', 'B.GLY.13', 'B.ASN.14', 'B.LEU.15', 'B.PRO.16'
    ]
...
def test_get_file_str_build_ligand():
    ...
    stru: Structure = sp.get_structure(pdb_file_path)
    stru = so.remove_solvent(stru)
    #print(sp.get_file_str(stru))
    #print(sp.get_file_str(stru.ligands[0]))
    assert False
    #with open(answer_file_path) as f:
