...
def geno_enzy_htp_workflow(stru_parser: StructureParser):
    ...
    """
    candidate_seq = [
        'read_file_from_somewhere.fasta',
    ]
    for seq in candidate_seq:
        raw_stru: Structure = stru_parser.get_structure(
            seq)  # structural prediction seq -> stru

        stru_prepare_problems: List[StruProblem] = eh.detect_stru_problems(
            raw_stru, stru_parser)  # pass stru_parser in this case problems are settled
        # 1
        prepped_stru = eh.prepare(
            raw_stru, stru_prepare_problems)  # changes in the same stru but change name
        # 2
